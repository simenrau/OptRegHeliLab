...
tic
fun = @(z) 0.5*z'*Q*z; 
options = optimoptions('fmincon','Algorithm', 'interior-point', 'MaxFunEvals',90000);

[Z, ZVAL, EXITFLAG] = fmincon(fun,z_0,[],[],A_eq,B_eq,vlb,vub,@Nonlincon1,options); % hint: quadprog. Type 'doc quadprog' for more info 
toc;

%% Extract control inputs and states
u_star1  = [Z(N*mx+1:mu:N*(mx+mu));Z(N*(mx+mu)-1)]; % Control input from solution
u_star2  = [Z(N*mx+2:mu:N*(mx+mu));Z(N*(mx+mu))];
