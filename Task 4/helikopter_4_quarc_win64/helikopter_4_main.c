 ...
 *
<<<<<<< HEAD
 * Date:           Tue Mar 06 17:40:03 2018
 * Model version:  1.185
=======
 * Date:           Tue Feb 27 21:35:30 2018
 * Model version:  1.180
...
typedef RT_MODEL_helikopter_4_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * helikopter_4(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(helikopter_4_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(helikopter_4_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://helikopter_4:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: helikopter_4 -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(helikopter_4_DW.HILReadEncoderTimebase_Task,
        (t_clock) helikopter_4_P.HILReadEncoderTimebase_clock, 500.0, -1)
        ;
      if (result == 0) {
