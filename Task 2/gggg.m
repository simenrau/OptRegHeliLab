...
plot(t,zeros(length(t),1)-30,'--b'); hold on; hold on; plot(t2,u_est*(180/pi)); grid
legend('u','upper constraint','lower constraint','Location','NorthEast'); ylabel('u'); title('q = 1','FontSize',13);
subplot(512)
plot(t,x1,'r',t,x1,'ko'); hold on; plot(t2,travel); grid
ylabel('\lambda')
subplot(513)
plot(t,x2,'r',t,x2','ko'); hold on; plot(t2,travel_rate); grid
ylabel('r')
subplot(514)
plot(t,x3,'r',t,x3,'ko'); hold on; plot(t2,pitch); grid
ylabel('p')
subplot(515)
plot(t,x4,'r',t,x4','ko'); hold on; plot(t2,pitch_rate); grid
