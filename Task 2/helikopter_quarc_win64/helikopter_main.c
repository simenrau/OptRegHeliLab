...
typedef RT_MODEL_helikopter_T RT_MODEL;
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(helikopter_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(helikopter_DW.HILReadEncoderTimebase_Task,
        (t_clock) helikopter_P.HILReadEncoderTimebase_clock, 500.0, -1)
        ;
      if (result == 0) {
